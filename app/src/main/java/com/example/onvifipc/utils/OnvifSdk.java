...
import com.example.onvifipc.onvif.GeNetWorkInterfaceThread;
import com.example.onvifipc.onvif.GetDeviceInfoThread;
import com.example.onvifipc.onvif.GetImagingSettingsThread;
import com.example.onvifipc.onvif.GetSnapshotInfoThread;
import com.example.onvifipc.onvif.SetImagingSettingsThread;
import com.example.onvifipc.onvif.SetNetworkInterfaceThread;
import com.example.onvifipc.onvif.SetSystemDateAndTimeThread;
import com.example.onvifipc.onvif.SetUserThread;
import com.example.onvifipc.onvif.StartFirmwareUpgradeThread;
import com.example.onvifipc.onvif.SystemRebootThread;
 ...
 * 本sdk集成
 * 截图功能 {@linkplain #getSnapshot(Context, String, String, String, GetSnapshotInfoThread.GetSnapshotInfoCallBack)}
 ...
 * 修改Ip地址 {@linkplain #setNetworkInterface(Context, String, String, SetNetworkInterfaceThread.SetNetworkInterfaceCallBack)}
...
public class OnvifSdk {
    ...
    public static String MONITOR_CAMERA_PSD = "12345";
    ...
    public synchronized static void findDevice(Context context) {
        ...
        findDevice(context, "192.168.1.255", new FindDevicesThread.FindDevicesListener() {
            ...
            public void searchResult(ArrayList<Device> devices) {
              ...
              //  ToastUtils.showToast(context, "sss");
                if (devices != null) {
                    for (Device device : devices) {
                        if (device.getServiceUrl() != null && device.getIpAddress() != null) {
                            String ip = device.getIpAddress();
                            ip = ip.split(":")[0];
                            if (ip.equals("192.168.1.160")) { // 设置海康摄像头用户名密码，海康摄像头兼容问题，暂时还不能使用onvif方法
                                device.setUserName(MONITOR_CAMERA_USER);
                                device.setUserPwd(MONITOR_CAMERA_PSD);
                            } else {
                                device.setUserName("admin"); // 设置主辅摄用户名密码
                                device.setUserPwd("12345");
                            }

                            String hostIP = NetUtil.getHostIP();
                            Log.d("TAG", "searchResult: hostIp===" + hostIP);
...
//
//                                        Log.d(tag, "发现摄像头设备" + JSONObject.toJSONString(device));
//                                    } else {
...
//            creatDevice(ipAdress, user, pd);
   ...
   /**
    ...
    public static void getDeviceInfo(final Context context, final Device device, final GetDeviceInfoThread.GetDeviceInfoCallBack callBack) {
     ...
     * @param pwd      pwd
     */
    private static void createDevice(String ipAdress, String username, String pwd) {
        Device device = new Device();
        device.setUserPwd(pwd);
